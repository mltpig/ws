...
class ConfigRoleChara
{
    ...
    public function getIdName($id,$belong): string
    {
        return $id.'-'.$belong;
    }
    ...
    public function initTable():void
    {
        ...
        foreach ($tableConfig as $key => $value) 
        {
            ...
            if($value['get_type'] == 2)
            {
                ...
                //只有官职需要
                $idName = $this->getIdName($value['unlock'],$value['belong']);
                $table->set($idName,[ 'id'     => $value['id'] ]);
    ...
    public function getOne(int $type ,$belong =false):array
    {
        if(!$belong){
            $idName = $type;
        }else{
            $idName = $this->getIdName($type,$belong);
        }

        $table = TableManager::getInstance()->get($this->tableName);
        if(!$table->count()) $this->initTable();
        
        $data = $table->get($idName);
