...
class Request
{
    ...
    public function xmlHttp(string $url,string $xml,array $headers = [],array $cookie = [] ,int $ms = 0):array
    {
        ...
        $response = $client->postXml($xml);

        return [$response->xml(true) , $response->getBody() ];
